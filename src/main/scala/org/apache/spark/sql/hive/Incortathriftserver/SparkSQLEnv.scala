  ...
  def init(optimization: String = "vanilla") {
    if (sqlContext == null) {
      ...
      if (optimization == "barq") {
        sqlContext.experimental.extraStrategies = sqlContext.experimental.extraStrategies :+ PredefinedJoinQueryPlanOptimization.asInstanceOf[Strategy]
        sqlContext.experimental.extraOptimizations = sqlContext.experimental.extraOptimizations :+ PredefinedJoinStrategy.asInstanceOf[Rule[LogicalPlan]]
      }

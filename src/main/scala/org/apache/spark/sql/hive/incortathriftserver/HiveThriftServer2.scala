...
import org.apache.spark.sql.hive.incortathriftserver.ui.ThriftServerTab
...
object HiveThriftServer2 extends Logging {
  ...
  def main(args: Array[String]) {
    ...
    logInfo(s"Starting Optimized SparkContext using $optimization")
    SparkSQLEnv.init(optimization)
  ...
  private[incortathriftserver] class SessionInfo(
  ...
  private[incortathriftserver] object ExecutionState extends Enumeration {
    val STARTED, COMPILED, FAILED, FINISHED = Value
    type ExecutionState = Value
  }

  private[incortathriftserver] class ExecutionInfo(
   ...
   */
  private[incortathriftserver] class HiveThriftServer2Listener(
